...
already_AddRefed<Image> RemoteMediaManagerChild::ReadbackYCbCr(
    const SurfaceDescriptorGPUVideo& aSD, ColorSpace2 aColorPrimaries) {
  SurfaceDescriptor sd;
  ...
  if (sd.get_SurfaceDescriptorBuffer().desc().type() !=
    ...
    LOGE("Unexpected BufferDescriptor type in ReadbackYCbCr");
    DeallocShmem(shmem);
    return nullptr;
  }
  RemoteImageHolder holder(std::move(sd), aColorPrimaries);
