...
bool VideoFrame::Resource::CopyPlaneInto(const Format::Plane& aPlane,
  ...
  if (source->GetFormat() == ImageFormat::PLANAR_YCBCR) {
    ...
    const gfx::IntSize chromaOffset = gfx::ChromaSize(
        gfx::IntSize(originOffset.x, originOffset.y), data->mChromaSubsampling);
    const gfx::IntRect cbcrRect =
        aRect + gfx::IntPoint(chromaOffset.width, chromaOffset.height);
    switch (aPlane) {
      case Format::Plane::Y:
